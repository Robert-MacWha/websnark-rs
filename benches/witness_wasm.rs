...
const _: () = {
    #[unsafe(export_name = "__wbgb__witness_wasm::bench_calculate_witness")]
    extern "C" fn __wbgt_test(cx: &::wasm_bindgen_test::__rt::Context) {
        cx.execute_sync(
            "witness_wasm::bench_calculate_witness",
            || {
                let mut c = Criterion::default()
                    .with_location("benches/witness_wasm.rs", "witness_wasm")
                    .sample_size(10)
                    .measurement_time(std::time::Duration::from_secs(60));
                bench_calculate_witness(&mut c);
            },
            None,
            None,
        );
...
fn bench_calculate_witness(c: &mut Criterion) {
    ...
    c.bench_function("calculate_witness", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                calculate_witness(black_box(circuit), black_box(input)).unwrap();
                total += start.elapsed();
            }
